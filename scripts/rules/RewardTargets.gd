...
## Boosters (BOxx) and shop items (SHxx) checked before modifiers (Mxx) —
## not because of any real ambiguity (no real ID matches more than one of
## these three patterns) but so a typo'd "M" family prefix can never shadow
## the two two-letter ones by accident.
