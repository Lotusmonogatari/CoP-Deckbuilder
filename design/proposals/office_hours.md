...
| # | Point | Status |
|---|---|---|
| 1 | What a `shop_item` reward/penalty DOES at runtime | **Resolved 2026-09-25, then fully answered by a separate, later proposal.** The narrower sub-question this row used to raise — what a stage-scoped consumable actually is, and how "one randomly-selected X from a tier" is expressed — is **built**, not open: see `design/proposals/inventory.md`, Cameron's own answers there (its §0/§0.1), and its Q1–Q8. A `shop_item` Reward/Penalty now adds the item to `GameState.inventory` (Q7, "into the bag") rather than applying it on the spot; using it later applies its own `Grants`, which can name a stage-effect token (`ENERGY`/`GUARD`/`DRAW`/`TURNS`/`GAFFE_CAP`) or a `TIER:` pool for a random pick from a tier, both live in the real Shop tab today (SH01–SH08, SH20–SH26). This row is left here, corrected, only so a reader who remembers it as open does not have to go looking for why it no longer is. |
